...
activateTab(targetIndex);var activeTab=$(tabs[targetIndex]);activeTab.find('.tab-border').css({visibility:"hidden"});smoothScrollTabs(tabs[targetIndex]);$(".ha-loop-tab-bgcontainer").removeClass("ha-loop-tab-active-bg");$(".ha-loop-tab-bgcontainer-".concat(targetIndex)).addClass("ha-loop-tab-active-bg");requestAnimationFrame(function(){startSvgAnimation((targetIndex+1)%tabs.length)})}};var startSvgAnimation=function startSvgAnimation(index){if(hoverMode||animationInProgress)return;tabs.removeClass("filling");var $nextTab=$(tabs[index]);var svgElement=$nextTab.find('.tab-svg')[0];if(!svgElement)return;if(window.innerWidth<=767){svgElement.setAttribute('width',itemWidth);svgElement.setAttribute('height',55);svgElement.setAttribute('viewBox',"0 0 ".concat(itemWidth," 55"));svgElement.style.width=itemWidth+'px';svgElement.style.height=55+'px'}else{svgElement.setAttribute('width',itemWidth);svgElement.setAttribute('height',itemHeight+2);svgElement.setAttribute('viewBox',"0 0 ".concat(itemWidth," ").concat(itemHeight));svgElement.style.height=itemHeight+2+'px'}
var borderElement=$nextTab.find('.tab-border')[0];if(!borderElement)return;$nextTab.find('.tab-border').css("visibility","visible");if(borderElement.getBBox().width!==0){if(window.innerWidth<=767){strokeWidth=2;borderElement.style.width=itemWidth-3+'px';borderElement.style.height=52+'px';borderElement.style.x=1;borderElement.style.y=1;borderElement.setAttribute('x',1);borderElement.setAttribute('y',1);borderElement.setAttribute('rx',strokeRY);borderElement.setAttribute('ry',strokeRY);borderElement.style.rx=strokeRY;borderElement.style.ry=strokeRY}else{if(strokeWidth>4){borderElement.setAttribute('x',3);borderElement.setAttribute('y',3)}else if(strokeWidth>2){borderElement.setAttribute('x',2);borderElement.setAttribute('y',2)}else{borderElement.setAttribute('x',1);borderElement.setAttribute('y',1)}
